...
const cookiesEnabled = inject('cookiesEnabled');
const setCookiesEnabled = inject('setCookiesEnabled');
...
<template>
  ...
  <nav>
    ...
    <RouterLink class="ml-3" to="/premium">Go to premium</RouterLink>

    <button
      id="consent-button"
      class="ml-3"
      @click="() => setCookiesEnabled(true)"
    >
      Give consent
    </button>
    ...
    <div class="mt-3">
      <div><b>Consent given:</b> {{ cookiesEnabled }}</div>
      <div><b>onIdentityAvailable:</b> {{ JSON.stringify(identity) }}</div>
      <div><b>onReady:</b> {{ JSON.stringify(ready) }}</div>
      <div><b>Mounted: </b>{{ JSON.stringify(mounted) }}</div>
    </div>
  </nav>
</template>